...
class SnowflakeTarget(luigi.Target):
    ...
    def exists(self, connection=None):
        ...
        try:
            ...
            cursor = connection.cursor()
            query = """SELECT 1 FROM {database}.{schema}.{marker_table} WHERE update_id='{update_id}'
            AND target_table='{database}.{schema}.{table}'""".format(
                database=self.sf_database,
                schema=self.schema,
                marker_table=self.marker_table,
    ...
    def clear_marker_table(self, connection):
        ...
        if self.marker_table_exists(connection):
            query = "DELETE FROM {database}.{schema}.{marker_table} where target_table='{database}.{schema}.{table}'".format(
                database=self.sf_database, schema=self.schema, marker_table=self.marker_table, table=self.table,
            )
            _execute_query(connection, query)
...
class SnowflakeLoadTask(SnowflakeLoadDownstreamMixin, luigi.Task):
    ...
    def create_database(self, connection):
        query = "CREATE DATABASE IF NOT EXISTS {database}".format(database=self.sf_database)
        _execute_query(connection, query)

    def create_schema(self, connection):
        query = "CREATE SCHEMA IF NOT EXISTS {database}.{schema}".format(database=self.sf_database, schema=self.schema)
        _execute_query(connection, query)
    ...
    def create_stage(self, connection):
        ...
        query = """
        CREATE OR REPLACE STAGE {database}.{schema}.{table}_{date}_stage
            URL = '{stage_url}'
